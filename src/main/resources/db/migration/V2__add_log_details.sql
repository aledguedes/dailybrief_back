...
-- Adiciona as novas colunas à tabela tbl_log
...
-- IMPORTANTE:
-- Se você já tem dados na tabela tbl_log e as colunas foram adicionadas com NOT NULL,
...
ALTER TABLE tbl_log ALTER COLUMN report_id DROP DEFAULT;
ALTER TABLE tbl_log ALTER COLUMN level DROP DEFAULT;
ALTER TABLE tbl_log ALTER COLUMN details DROP DEFAULT;

-- Se você precisar adicionar uma restrição UNIQUE para report_id (baseado na sua necessidade específica):
-- ALTER TABLE tbl_log ADD CONSTRAINT uq_tbl_log_report_id UNIQUE (report_id);
